        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-12">
            ...
            <div className="rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-card)]">
              ...
              <div className="mt-4 rounded-xl bg-muted/40 p-4">
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  Loyer estimé : {fmtEUR(equivalentRent)} / mois
                </div>
              </div>
            </div>
          </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className={`rounded-full px-2 py-0.5 text-xs ${
                          maintenanceAuto ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"
                        }`}
                      >
                        auto
                      </button>
                      <span className="text-sm font-semibold tabular-nums">
                        {fmtEUR(effectiveMaintenance)}
                      </span>
                    </div>
                  </div>
                  {!maintenanceAuto && (
          ...
          <div className="space-y-5 lg:col-span-4">
            <div className="grid grid-cols-2 gap-4">
              <StatCard
                label="Coût total achat"
                value={fmtEUR(result.totalPrice)}
                sub="prix + 7,5% notaire"
              />
              <StatCard
                label="Mensualité totale"
                value={fmtEUR(Math.round(result.totalMonthly))}
                sub="crédit + assurance"
              />
              <StatCard
            ...
            {showHypo && (
              <div className="mt-3 rounded-2xl border border-border bg-card p-5 text-sm text-muted-foreground shadow-[var(--shadow-card)]">
                <ul className="list-disc space-y-2 pl-5">
                  ...
                  <li>
                    Patrimoine net = (valeur bien revalorisée − capital restant dû − coûts cumulés − apport) − (apport
                    + différentiel mensuel investis à {investReturn}% − loyers cumulés).
                  </li>
